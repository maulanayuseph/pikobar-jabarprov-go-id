...
export const actions = {
  async getItems ({ commit }, options) {
    const { data } = await axios.get('https://dashboard-pikobar-api.digitalservice.id/v2/sebaran/pertumbuhan', {
      headers: {
  ...
  async getItem ({ commit }, query, options) {
    ...
    const { data } = await axios.get('https://dashboard-pikobar-api.digitalservice.id/v2/sebaran/pertumbuhan?' + query, {
      headers: {
        'api-key': process.env.DASHBOARD_API_KEY
      }
    })
