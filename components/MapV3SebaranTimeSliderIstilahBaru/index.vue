...
export default {
  name: 'MapV3SebaranTimeSliderIstilahBaru',
  components: {
    ContentLoader,
    VueSlider
  },
  ...
  computed: {
    ...
    dataSebaranMarker () {
      return this.$store.getters['data-sebaran-marker-v2/itemMap']
    },
    isLoading () {
      return this.$store.getters['data-sebaran-marker-v2/isLoading']
    }
  },
  watch: {
  ...
  methods: {
      ...
      if (dataSebaranMarker !== undefined) {
        ...
        if (this.isGrouping) {
          ...
          jsonRegion.features.forEach((element) => {
            const clusterAktif = markerClusters[element.properties[keyRegion]].confirmation_diisolasi.addTo(this.map)
            const clusterSembuh = markerClusters[element.properties[keyRegion]].confirmation_selesai.addTo(this.map)
            const clusterMeninggal = markerClusters[element.properties[keyRegion]].confirmation_meninggal.addTo(this.map)

            this.dataLayer.push(clusterAktif)
            this.dataLayer.push(clusterSembuh)
            this.dataLayer.push(clusterMeninggal)

            this.createClusterPopup(element, keyRegion, 'confirmation_diisolasi')
            this.createClusterPopup(element, keyRegion, 'confirmation_selesai')
            this.createClusterPopup(element, keyRegion, 'confirmation_meninggal')
          })
        }
      }
    },
    ...
    paramMarkerCluster () {
      return {
        gabungan_aktif: this.$L.markerClusterGroup(this.configCluster('cluster-gabungan-aktif')),
        confirmation_total: this.$L.markerClusterGroup(this.configCluster('cluster-confirmation-total')),
        confirmation_diisolasi: this.$L.markerClusterGroup(this.configCluster('cluster-confirmation-diisolasi')),
    ...
    getDataSebaranMarker (region, regionCode = '') {
      ...
      query += '&category=confirmation_total'
      ...
      this.$store.dispatch('data-sebaran-marker-v2/getItem', query)
    }
  }
}
</script>
